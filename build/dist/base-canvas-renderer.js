export class BaseCanvasRenderer {
  ...
  beginPath(point) {
    if (!this._currentPath) this._currentPath = new Path();
    ...
    if (fromIndex >= 0) this._paths.splice(fromIndex, this._paths.length);
  }

  addToPath(points, predictedPoints) {
    if (!points || !this._currentPath) return;
  ...
  endPath(point) {
    if (this._drawPredictedEvents) this.clearPredictionCanvas();
