class HIDUSI {
  ...
  get productName() {
    if (this._usiDevice)
      return this._usiDevice.productName;
    else
      return null;
  }
  ...
  constructor() {
    if (typeof window.navigator.hid === 'undefined')
      throw 'WebHID is not supported in this browser';
  ...
  open = async () => {
    ...
    if (!this._usiDevice.opened)
        await this._usiDevice.open();

    let collection = this._findCollectionByUsage(this._usiDevice, usagePage, usage);
  ...
  setPreferredColor = async (index, color) => {
  ...
  _findCollectionByUsage = function(device, usagePage, usage) {
    return device.collections.find(candidate =>
      candidate.usagePage === usagePage && candidate.usage === usage);
  }

  _findFeatureReportByUsage = function(item, usage) {
  ...
  _findInputReportByUsage = function(item, usage) {
